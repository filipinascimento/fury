...
def heliosLayout(iterationCount,edgesArray,pos,velocities):
    for i in range(iterationCount):
        helios.layoutDelayed(edgesArray,pos,velocities);
...
if(mode == 0):
    ...
    vertices_count = 1000
    view_size = 100
    network = nx.random_geometric_graph(vertices_count, 0.05)
    positions = view_size * \
...
def new_layout_timer(showm, edges_list, vertices_count,
    ...
    velocities = np.ascontiguousarray(np.zeros((vertices_count, 3),dtype=np.float32));
    edgesArray = np.ascontiguousarray(np.array(edges_list),dtype=np.uint64);

    # helios.startAsyncLayout(edgesArray,pos,velocities);
